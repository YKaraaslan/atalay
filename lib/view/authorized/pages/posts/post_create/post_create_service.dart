...
Future<bool> savePostToDatabase(PostModel model, List<File> images) async {
  ...
  try {
    await ServicePath.postsCollectionReference.add(model.toMap()).then((value) async {
      await ServicePath.postsCollectionReference.doc(value.id).update({'postID': value.id});
      ...
      await ServicePath.postsCollectionReference.doc(value.id).update({'images': imageURLs});
    });
    return true;
  } on Exception {
    return false;
